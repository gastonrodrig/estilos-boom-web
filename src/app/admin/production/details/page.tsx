    ...
    <section className="mx-auto max-w-7xl space-y-8 px-6 py-10 bg-[#fdfcfc] relative">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <select value={selectedWorkshop} onChange={(e) => setSelectedWorkshop(e.target.value)} className="h-16 px-6 rounded-2xl border border-rose-100 bg-white text-sm font-medium text-[#594246] outline-none shadow-sm cursor-pointer appearance-none min-w-[180px]">
            {workshops.map(w => <option key={w} value={w}>{w}</option>)}
          </select>

          <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="h-16 px-6 rounded-2xl border border-rose-100 bg-white text-sm font-medium text-[#594246] outline-none shadow-sm cursor-pointer appearance-none min-w-[160px]">
        ...
        {Object.entries(counts).map(([key, count]) => (
          <button
            ...
            className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg px-4 py-2 text-[11px] font-bold transition-all uppercase tracking-wider ${
              filter === key ? "bg-[#F2778D] text-white shadow-sm" : "border border-rose-100 bg-white text-[#9b8088] hover:bg-rose-50"
            }`}
          >
          ...
          if (filtered.length === 0) {
            return (
              <div className="py-24 text-center border-2 border-dashed border-rose-100 rounded-[40px] bg-white/50 backdrop-blur-sm space-y-4">
                <div className="bg-rose-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6"><Package className="h-10 w-10 text-rose-200" /></div>
                <h3 className="text-xl font-bold text-[#594246]">No se encontraron órdenes</h3>
                <p className="text-[#9b8088] max-w-xs mx-auto text-sm leading-relaxed">No hay procesos en la etapa <span className="text-[#F2778D] font-bold">"{filter}"</span>.</p>
              </div>
            );
          }
          ...
          <div className="space-y-1 sm:space-y-2 flex-1">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                <span className={`rounded-md px-2 py-0.5 sm:px-3 sm:py-1 text-[11px] sm:text-[13px] font-normal text-white ${order.status === 'CONTROL_CALIDAD' ? 'bg-green-500' : 'bg-[#F291A3]/80'}`}>
                  ...
                  if (localEstimatedDate?.startsWith(today)) {
                    return (
                      <span className="rounded-md bg-rose-100 px-2 py-0.5 sm:px-3 sm:py-1 text-[11px] sm:text-[13px] font-bold text-[#F2778D] flex items-center gap-1.5 border border-rose-200">
                        <Clock className="w-3 h-3" /> ENTREGA HOY
                      </span>
                    );
                  }
                  return null;
        ...
        {isOpen && (
          ...
          >
                  ...
                  <div key={wIdx} className="mb-8 last:mb-0">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
                      <h4 className="text-lg font-medium text-[#594246]">
                        Detalle de Production {workshopsToRender.length > 1 && <span className="text-[#F2778D]">· {agentName}</span>}
                      </h4>
                    </div>
                          ...
                          {order.base_items?.map((item: any, idx: number) => {
                            ...
                            return (
                              <tr key={idx} className="border-b border-rose-50/50 hover:bg-rose-50/30 transition-colors">
                                ...
                                {/* 🛠️ SECCIÓN CORREGIDA: Resiste strings y objetos de color sin reventar */}
                                <td className="py-4 px-2">
                                  {item.id_variant?.color?.name || 
                                   item.id_variant?.color_name || 
                                   (typeof item.id_variant?.color === 'object' ? item.id_variant?.color?.name : item.id_variant?.color) || 
                                   "Sin color"}
                                </td>

                                <td className="py-4 px-2 text-center font-bold">{item.quantity}</td>
                                <td className="py-4 px-2 text-center text-[#9b8088]">
                                  {isPending && unitCost === 0 ? <span className="text-rose-400 font-bold italic">Pendiente</span> : `S/ ${unitCost.toFixed(2)}`}
                                </td>
                                <td className="py-4 px-2 text-right font-bold text-[#F2778D]">
                                  {isPending && unitCost === 0 ? <span className="text-rose-400 font-bold italic">S/ 0.00</span> : `S/ ${(item.quantity * unitCost).toFixed(2)}`}
                                </td>
                              </tr>
                            );
                          })}
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-4">
              <CTA onClick={() => setActiveModal("TECH")} className="!bg-white border border-[#F2778D] !text-[#F2778D] !py-3 !px-6" icon={Eye}>
                Ficha Técnica
              </CTA>
              ...
              {(order.status === "CONTACTO_INICIAL" || order.status === "COMPARANDO") && (
                <>
                  <CTA onClick={() => setActiveModal("COST")} className="!bg-white border border-[#594246] !text-[#594246] !py-3 !px-8 shadow-sm" icon={FileText}>
                    Registrar Costos de Taller
                  </CTA>
                  <CTA onClick={() => { onUpdateStatus(order._id, "RECHAZADA"); toast.success("Orden rechazada exitosamente."); }} className="!bg-white border border-[#594246] !text-[#594246] !py-3 !px-8 shadow-sm" icon={XCircle}>
                    Rechazar Orden
                  </CTA>
                  {displayTotal > 0 && (
                    <CTA onClick={() => setActiveModal("WINNER")} className="!py-3 !px-8 shadow-lg shadow-rose-100" icon={CheckCircle2}>
                      Confirmar Taller
                    </CTA>
                  )}
                </>
              ...
              {order.status === "RECHAZADA" && (
                <div className="flex w-full items-center justify-between mt-2 border-t border-rose-50 pt-4">
                  <div className="flex items-center gap-2 text-rose-500 font-bold text-sm bg-rose-50 px-4 py-2 rounded-xl">
                    <Clock className="w-4 h-4" /> Se eliminará en 3 días
                  </div>
                  <CTA onClick={() => { onUpdateStatus(order._id, "CONTACTO_INICIAL"); toast.success("Orden reanudada."); }} className="!bg-white border border-[#10b981] !text-[#10b981] !py-3 !px-8 shadow-sm hover:!bg-[#10b981] hover:!text-white transition-all" icon={ArrowRight}>
                    Continuar Orden
                  </CTA>
                </div>
              )}
              ...
              {order.status === "CONTROL_CALIDAD" && (
                <CTA onClick={() => { onUpdateStatus(order._id, "COMPLETADA"); toast.success("Orden completada e ingresada a inventario."); }} className="!py-3 !px-8 shadow-lg shadow-rose-100" icon={CheckCircle2}>
                  Completar Orden
                </CTA>
              )}
            </div>
