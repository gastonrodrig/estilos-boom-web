...
export default function CollectionPage() {
  ...
  const headerContent = useMemo(() => {
    const titles = {
      "new-in": { title: "Nueva Colección Primavera 2026", sub: "Descubre las últimas tendencias en moda femenina." },
      "best-seller": { title: "Best Sellers Favoritos", sub: "Las prendas más amadas por nuestra comunidad." },
      "dresses": { title: "Vestidos Exclusivos", sub: "Diseños pensados para resaltar tu esencia." },
    };
    return titles[type as keyof typeof titles] || { title: "Colección Boom", sub: "Moda que inspira." };
  }, [type]);

  return (
    <div className="min-h-screen bg-white">
      <div className="bg-[#f8b4c2] text-white py-2 text-center text-xs font-medium tracking-wide">
        ✨ Envío gratis en compras mayores a S/149 | Descuentos hasta 40% en toda la tienda
      </div>

      <header className="bg-gradient-to-b from-[#fce4ec] to-white pt-24 pb-16 px-6 text-center">
        <h1 className="text-4xl md:text-5xl font-serif text-[#4a4a4a] mb-4">{headerContent.title}</h1>
        <p className="text-gray-500 max-w-2xl mx-auto italic">{headerContent.sub}</p>
      </header>

      <main className="container mx-auto px-6 pb-20">
        <div className="flex justify-between items-center py-6 border-b border-gray-100 mb-10">
          <div className="flex items-center gap-6">
            <button onClick={() => setIsFilterOpen(true)} className="flex items-center gap-2 border px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50 transition">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12">
          {loading ? (
            Array(8).fill(0).map((_, i) => (
              <div key={i} className="animate-pulse bg-gray-100 aspect-[3/4] rounded-lg" />
            ))
          ) : (
            products.map((product) => (
              <ProductCardCatalogue key={product.id_product} product={product} />
            ))
          )}
        </div>
      </main>
