...
export default function DispatchConfirmationPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  // Evidences photos and files
  const [incidentPhotos, setIncidentPhotos] = useState<Record<string, string>>({});
  const [dispatchPhotoName, setDispatchPhotoName] = useState<string>("");
  const [dispatchDocName, setDispatchDocName] = useState<string>("");
  ...
  const handleConfirm = async () => {
    ...
    if (dispatchPhotoFile) filesToUpload.push(dispatchPhotoFile);
    if (dispatchDocFile) filesToUpload.push(dispatchDocFile);
    Object.values(incidentPhotoFiles).forEach((file) => {
      if (file) filesToUpload.push(file);
    });

    if (filesToUpload.length > 0) {
      const uploadToast = toast.loading("Subiendo evidencias de despacho al servidor...");
      try {
  ...
  if (!doc) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-[#8C6B79] text-sm italic">Cargando documento de despacho…</p>
      </div>
    );
  }

  const srcName = doc.id_source_warehouse?.name?.replace(/_/g, " ") ?? "Almacén Central";
  const tgtName = doc.id_target_warehouse?.name?.replace(/_/g, " ") ?? "Venta Web (Cliente)";
    ...
    return (
      <div className="w-full min-h-screen bg-[#F7EEF1] dark:bg-transparent flex items-center justify-center p-4 font-sans transition-colors duration-300">
        <div className="bg-white dark:bg-black/50 backdrop-blur-2xl rounded-2xl p-8 md:p-12 shadow-[0_8px_30px_rgba(242,119,141,0.06)] dark:shadow-none border border-[#EEDCE1] dark:border-white/5 max-w-md w-full text-center animate-in zoom-in-95 duration-500">
          ...
          <h1 className="text-2xl font-bold font-serif text-[#5B283A] dark:text-[#Fdfcfc] mb-2 transition-colors duration-300">¡Despacho Confirmado!</h1>
          <p className="text-[#844C60] dark:text-[#C9B3BC] text-sm mb-8 transition-colors duration-300 leading-relaxed">
            La salida de mercadería para el despacho <span className="font-bold text-[#40202D] dark:text-white">#{doc.document_number}</span> ha sido procesada exitosamente.
            La orden asociada ahora se encuentra en estado **Preparando**.
          </p>
          ...
          <div className="bg-[#FCF8F9] dark:bg-white/5 border border-[#EEDCE1] dark:border-white/5 rounded-xl p-5 text-left mb-8">
            ...
            <ul className="space-y-3 text-sm">
              ...
              <li className="flex justify-between items-center pt-3 border-t border-[#EEDCE1] dark:border-white/10">
                <span className="text-[#844C60] dark:text-[#C9B3BC] font-medium text-[13px]">Total despachado:</span>
                <span className="text-[#D6405F] dark:text-[#F2778D] font-bold text-lg">{totalReceived} / {totalExpected} uds.</span>
              </li>
            </ul>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link href="/storekeeper/warehouse/dispatches" className="w-full flex items-center justify-center gap-2 bg-[#5B283A] text-white hover:bg-[#40202D] rounded-xl px-6 py-3.5 font-medium transition-all duration-300 transform hover:-translate-y-1">
  ...
  return (
    <div className="w-full min-h-screen bg-[#F7EEF1] dark:bg-transparent p-4 pt-24 md:p-8 md:pt-28 font-sans transition-colors duration-300">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
            <div>
              ...
              <p className="text-[14px] text-[#844C60] dark:text-[#C9B3BC] mt-1.5 font-medium flex items-center gap-2">
                Origen: {srcName} <ArrowLeft className="w-3 h-3 rotate-180 text-[#D6405F]" /> {tgtName}
              </p>
            </div>
        ...
        <div className="bg-white dark:bg-black/50 backdrop-blur-2xl rounded-2xl shadow-[0_8px_30px_rgba(242,119,141,0.06)] dark:shadow-none border border-[#EEDCE1] dark:border-white/5 mb-8 overflow-hidden transition-colors duration-300">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-[15px] text-[#40202D] dark:text-white">Ver Artículos Vendidos</span>
            </div>
            <ChevronDown className={`w-5 h-5 text-[#844C60] dark:text-[#F2B6C1] transition-transform duration-300 ${showItemsDetail ? 'rotate-180' : ''}`} />
          </button>
          ...
          {showItemsDetail && (
            <div className="px-6 md:px-8 pb-8 pt-4 border-t border-[#EEDCE1] dark:border-white/5">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
                ...
                <div>
                  <p className="text-[11px] uppercase tracking-wider font-bold text-[#844C60] dark:text-[#C9B3BC] mb-1.5">Total Vendido</p>
                  <p className="text-[14px] font-bold text-[#40202D] dark:text-white">{totalExpected} uds.</p>
                </div>
                <div>
                  <p className="text-[11px] uppercase tracking-wider font-bold text-[#844C60] dark:text-[#C9B3BC] mb-1.5">Fecha del Pedido</p>
              ...
              <div className="bg-[#FCF8F9] dark:bg-white/5 rounded-xl p-5 border border-[#EEDCE1] dark:border-white/5">
                <p className="text-[11px] font-bold text-[#844C60] dark:text-[#C9B3BC] mb-3 uppercase tracking-wider">Artículos en la Orden</p>
        ...
        <h3 className="text-xl font-bold font-sans text-[#40202D] dark:text-white mb-5">Verificar Artículos a Despachar</h3>
            ...
            return (
              <div key={vId ?? idx} className="bg-white dark:bg-black/50 backdrop-blur-2xl rounded-2xl p-6 md:p-8 shadow-[0_8px_30px_rgba(242,119,141,0.06)] dark:shadow-none border border-[#EEDCE1] dark:border-white/5 transition-colors duration-300">
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                  <div>
                    <label className="block text-[11px] uppercase tracking-wider font-bold text-[#844C60] dark:text-[#C9B3BC] mb-2">Cantidad Vendida</label>
                    <input 
                      type="number" 
                  ...
                  <div>
                    <label className="block text-[11px] uppercase tracking-wider font-bold text-[#5B283A] dark:text-[#F2b6c1] mb-2">Cantidad Real Despachada</label>
                    <input 
                      type="number" 
                      min={0}
                ...
                <label className="flex items-center gap-3 cursor-pointer group w-fit">
                  ...
                  />
                  <span className="text-[14px] font-bold text-[#40202D] dark:text-white">¿Hubo alguna incidencia/merma al despachar?</span>
                </label>
                    ...
                    >
                      ...
                      {incidentReasons[vId] === "Otro motivo" && (
                        <div className="mb-6 animate-in fade-in slide-in-from-top-2 duration-300">
                          ...
                          <input 
                            ...
                            placeholder="Ej. La prenda presenta una mancha pequeña en la parte trasera"
                      ...
                      {currentQtyReceived < item.quantity_expected && (
                        <p className="text-xs text-amber-500 flex items-center gap-1 mt-4">
                          ...
                          Diferencia de {item.quantity_expected - currentQtyReceived} unidad(es) no despachada(s).
                        </p>
                      )}
                    </motion.div>
                  )}
        ...
        {/* EVIDENCIAS GLOBALES */}
        <h3 className="text-xl font-bold font-sans text-[#40202D] dark:text-white mb-5">Evidencias del Despacho</h3>
        ...
        <div className="bg-white dark:bg-black/50 backdrop-blur-2xl rounded-2xl p-6 md:p-8 shadow-[0_8px_30px_rgba(242,119,141,0.06)] dark:shadow-none border border-[#EEDCE1] dark:border-white/5 mb-10 transition-colors duration-300">
          ...
          <input 
            type="file" 
            id="dispatch-photo-input" 
            accept="image/*" 
            className="hidden" 
            onChange={(e) => { 
              const file = e.target.files?.[0]; 
              if (file) { 
                setDispatchPhotoName(file.name); 
          ...
          <input 
            type="file" 
            id="dispatch-doc-input" 
            accept="image/*,application/pdf" 
            className="hidden" 
            onChange={(e) => { 
              const file = e.target.files?.[0]; 
              if (file) { 
                setDispatchDocName(file.name); 
                setDispatchDocFile(file);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {/* Foto General */}
            <div 
              onClick={() => document.getElementById("dispatch-photo-input")?.click()} 
            ...
            >
              ...
              <span className="text-[15px] font-bold text-[#40202D] dark:text-white text-center mb-1.5">
                {dispatchPhotoName ? `Foto: ${dispatchPhotoName.slice(0, 20)}` : "Foto de Paquete/Caja"}
              </span>
              <span className="text-[12px] text-[#844C60] dark:text-[#C9B3BC] text-center font-medium">Evidencia física del estado del empaque</span>
            </div>

            {/* Documento */}
            <div 
              onClick={() => document.getElementById("dispatch-doc-input")?.click()} 
              className="flex flex-col items-center justify-center p-8 bg-[#FCF8F9] dark:bg-white/5 border border-dashed border-[#EEDCE1] dark:border-white/10 hover:border-[#D6405F] dark:hover:border-[#F2778D] rounded-xl cursor-pointer transition-colors group"
            >
              <div className="w-12 h-12 rounded-full bg-white dark:bg-[#F2778D]/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-sm">
              ...
              <span className="text-[15px] font-bold text-[#40202D] dark:text-white text-center mb-1.5">
                {dispatchDocName ? `Doc: ${dispatchDocName.slice(0, 20)}` : "Guía de Remisión / Comprobante"}
              </span>
        ...
        <div className="flex justify-between items-center pb-12">
          <Link href="/storekeeper/warehouse/dispatches"
          ...
          >
            {loading ? "Procesando…" : doc.status === "COMPLETADO" ? "Ya despachado" : "Confirmar Despacho de Venta"}
          </button>
        </div>
